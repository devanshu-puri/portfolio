...
const CrowdCanvas = ({ src, rows = 15, cols = 7 }) => {
  ...
  useEffect(() => {
    ...
    const createPeep = ({ image, rect }) => ({
        image, rect, width: rect[2], height: rect[3],
        x: 0, y: 0, anchorY: 0, scaleX: 1, walk: null,
        render(ctx) {
            ctx.save();
            ctx.translate(this.x, this.y);
            ctx.scale(this.scaleX, 1);
            ctx.drawImage(this.image, this.rect[0], this.rect[1], this.rect[2], this.rect[3], -this.width / 2, -this.height, this.width, this.height);
            ctx.restore();
        },
    });
